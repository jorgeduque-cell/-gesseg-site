...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ----- Lines slider (Líneas de negocio 360°) — scroll nativo fluido -----
  document.querySelectorAll('[data-slider]').forEach(slider => {
    const viewport = slider.querySelector('.lines-slider-viewport');
    ...
    const getItemWidth = () => {
      return items[0]?.getBoundingClientRect().width || viewport.offsetWidth;
    };

    const currentIndex = () => {
      const itemW = getItemWidth();
      return Math.round(viewport.scrollLeft / itemW);
    };

    const scrollToIndex = (idx) => {
      const itemW = getItemWidth();
      const max = total - 1;
      // Loop: si pasa del final vuelve al inicio (y viceversa)
      let target = idx;
      if (idx > max) target = 0;
      if (idx < 0) target = max;
      viewport.scrollTo({ left: target * itemW, behavior: 'smooth' });
    };
